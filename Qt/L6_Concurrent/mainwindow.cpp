...
void MainWindow::StartRace(void){
    if(ui->rb_qtConcur->isChecked()){
        auto fun1 = [this]{return concurRace1->DoWork(&number, false, ui->sb_initNum->value());};
        auto fun2 = [this]{return concurRace2->DoWork(&number, false, ui->sb_initNum->value());};

        QtConcurrent::run(fun1).then(fun2);
    }
    else{
